...
# Training settings
mlpLayer = 10                       # Number of hidden units per layer
bias = True                         # Choice of bias in the NN linear maps
onoffLambdaDecay = False            # Regularization decay term
epochs = 1000                        # Number of epochs per training round
outerTimes = 10                     # Number of training rounds
lr = 1e-2                           # Learning rate
lambdaDecayEpochs = 1200            # Lambda decay each'lambdaDecayEpochs'
gammaThreshold = 1e-4               # Threshold for gamma pruning
weightThreshold = 1e-4              # Threshold for weight pruning
trainBatchSize = 1024               # Choose batch size for each train

# Other relevant data parameter Setting
onoffDetrend = True                  # Detrend data (*-mean)
...
uEst = torch.tensor(loadData['uEst'], device=TORCH_MACHINE).float()
yEst = torch.tensor(loadData['yEst'], device=TORCH_MACHINE).float()
uVal = torch.tensor(loadData['uVal'], device=TORCH_MACHINE).float()
yVal = torch.tensor(loadData['yVal'], device=TORCH_MACHINE).float()
...
# NN Structure
layers = [uEstDataPro.shape[1], mlpLayer, mlpLayer, mlpLayer, yEstDataPro.shape[1]]
...
for repIndex in range(repeatExperiment):
    ...
    model = MLPNetwork(layers, activation=torch.relu, bias=bias,
    ...
    # Folder and filename
    folder = FOLDER_HEAD + 'CT_results_layers_' + str(layersN) + '_units_' + \
        ...
        for epoch in epochBar:
            ...
            if PLOT:
                plt.axis([0, outerTimes * epochs,
                          0, np.round(np.max(np.concatenate((predLosses, trainLosses))))])
