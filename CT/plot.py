...
lag = 20                            # Chosen lags for regressors
percentage = float(1)               # Percentage of data used
lag_input = lag_output = lag

onoff_detrend = True                  # Detrend data (*-mean)
onoff_normalize = False               # Normalize data (*-mean)/std

# Import training and testing data
loadData = scio.loadmat('CT_Data/dataBenchmark.mat')
u_est = torch.tensor(loadData['uEst'], device=TORCH_MACHINE).float()
y_est = torch.tensor(loadData['yEst'], device=TORCH_MACHINE).float()
u_val = torch.tensor(loadData['uVal'], device=TORCH_MACHINE).float()
y_val = torch.tensor(loadData['yVal'], device=TORCH_MACHINE).float()
...
rep_index = 10                       # Number of time to repeat experiment
layers = 3                           # Number of hidden layers
mlp_layer = 10                       # Number of hidden units per layer
outer_times = 10                     # Number of identification cycles
bias = True
activation = torch.relu

FOLDER_HEAD = './MLP/'
folder = FOLDER_HEAD + 'CT_results_layers_' + str(layers) + '_units_' + \
...
layers = [u_est_data_pro.shape[1], mlp_layer, mlp_layer, mlp_layer, y_est_data_pro.shape[1]]
...
# ax.set_title('Input Benchmark Dataset for CT Model Estimation')
ax.set_ylabel(r'pump voltage [V]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_est_input.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                          top=0.9, wspace=0.3, hspace=0.2)
plt.savefig("./figures/ct_input_estimation_data.pdf")
...
ax.grid(True, 'major', 'x')
# ax.set_title(' Input Benchmark Dataset for CT Model Validation')
ax.set_ylabel(r'pump voltage [V]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_val_input.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                          top=0.9, wspace=0.3, hspace=0.2)
plt.savefig("./figures/ct_input_validation_data.pdf")
...
ax.grid(True, 'major', 'x')
ax.grid(True, 'both', 'y')
# ax.set_title('One Step Ahead prediction results for CT Benchmark.')
ax.set_ylabel(r'water level [V]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_mlp0.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                    top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_mlp_pred_results_" + name + ".pdf")
...
fig_mlp1.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                         top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_mlp_sim_results_" + name + ".pdf")
...
fig_mlp3.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                         top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_mlp_validation_progress_" + name + ".pdf")

######################### Plot Sparsity MLP #################################
fig_mlp4 = plt.figure(figsize=[7, 4])
gs = fig_mlp4.add_gridspec(4, 7)
...
ax = fig_mlp4.add_subplot(gs[:, 2:4])
...
ax = fig_mlp4.add_subplot(gs[:, 4:6])
ax.spy(np.abs(mlp_model.weights['linear_2'].detach().cpu().numpy().transpose()),
       markersize=4)
ax.tick_params(axis='both', which='both', left=False, right=False,
               bottom=False, top=False, labelbottom=False, labelsize=8)
ax.set_title('Third MLP Layer', y=0, pad=-15, verticalalignment="top")
ax = fig_mlp4.add_subplot(gs[1:3, 6])
ax.spy(np.abs(mlp_model.weights['linear_3'].detach().cpu().numpy().transpose()),
       markersize=4)
ax.tick_params(axis='both', which='both', left=False, right=False,
...
# fig_mlp4.suptitle('Sparsity plots of the estimated MLP Network')
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_mlp_sparse_model_" + name + ".pdf")

##############################################################################
##############################################################################
############################### IMPORT LSTM ##################################

index_lambda = float(4)              # Regularization parameter
rep_index = 17                       # Number of time to repeat experiment
layers = 1                           # Number of hidden layers
lstm_layer = 50                      # Number of hidden units per layer
outer_times = 10                     # Number of identification cycles
bias = True
...
pred_val_mean[lag:], pred_val_std[lag:] = lib.predict_distribution(
    ...
    std_y, mean_y, repeat=10000)
lstm_loss_pred = torch.sqrt((torch.nn.functional.mse_loss(pred_val_mean, y_val[:, 0])))

fig_lstm0 = plt.figure(figsize=[7, 4])
ax = fig_lstm0.add_subplot(1, 1, 1)
bayes_str = 'Bayesian Model (RMSE = {:2.4f}'.format(lstm_loss_pred)
...
ax.grid(True, 'major', 'x')
ax.grid(True, 'both', 'y')
# ax.set_title('One Step Ahead prediction results of CT dataset.')
ax.set_ylabel(r'water level [V]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_lstm0.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                    top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_lstm_pred_results_" + name + ".pdf")

##################### Plot Simulation Validation ###########################
sim_val_mean, sim_val_std = lib.simulate_distribution(
    lstm_model, u_val_data, y_val_data, masks, sum_gamma, hessian_auto,
    std_y, mean_y, lag_input, lag_output, repeat=10000)
lstm_loss_sim = torch.sqrt((torch.nn.functional.mse_loss(sim_val_mean, y_val[:, 0])))

fig_lstm1 = plt.figure(figsize=[7, 4])
ax = fig_lstm1.add_subplot(1, 1, 1)
bayes_str = 'Bayesian Model (RMSE = {:2.4f}'.format(lstm_loss_sim)
true_sim1 = ax.plot(val_time, y_val.cpu().numpy(), color='tab:blue', label=r'True Output')
sim_regu1 = ax.plot(val_time, sim_val_mean.cpu().numpy(), '-',
                    color='tab:orange', label=bayes_str + ')')
ax.fill_between(val_time, sim_val_mean.cpu().numpy() - sim_val_std.cpu().numpy(),
...
fig_lstm1.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                          top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_lstm_sim_results_" + name + ".pdf")
...
fig_lstm2.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                          top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_lstm_training_progress_" + name + ".pdf")

######################### Plot Validation Progress ##########################
fig_lstm3 = plt.figure(figsize=[7, 4])
ax = fig_lstm3.add_subplot(1, 1, 1)
ax.axis([0, len(pred_vals), 0, 1])
ax.set_xticks(np.arange(0, outer_times+1, 1))
pred_prog = ax.plot(np.arange(outer_times) + 1, pred_vals, 'x',
                    color = 'tab:blue', linewidth=2,
                    label=r'One Step Ahead Prediction RMSE')
...
fig_lstm3.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                          top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_lstm_validation_progress_" + name + ".pdf")

######################### Plot Sparsity LSTM #################################
fig_lstm4 = plt.figure(figsize=[9, 5])
...
ax.spy(np.abs(lstm_model.weights['lstm_0.linear_ii'].detach().numpy().transpose()),
       markersize=4)
ax.set_yticks(np.arange(0, lag_input+lag_output, 10))
ax.tick_params(axis='both', which='both', left=False, right=False,
               bottom=False, top=False, labelbottom=False, labelsize=8)
ax.set_title(r'$W_{ii}$', y=0, pad=-15, verticalalignment="top")
ax.set_ylabel('Inputs z', fontsize=10)
...
fig_lstm4.subplots_adjust(left=0.08, bottom=0.05, right=0.98, top=0.95,
                          wspace=0.35, hspace=0.3)
# fig_lstm4.suptitle('Sparsity plots of the estimated LSTM Network')
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_lstm_sparse_model_" + name + ".pdf")
...
ax.grid(True, 'major', 'x')
ax.grid(True, 'both', 'y')
# ax.set_title('Simulation Comparison of CT validation dataset.')
ax.set_ylabel(r'water level [V]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_sim.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                        top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/ct_sim_comparison_" + name + ".pdf")