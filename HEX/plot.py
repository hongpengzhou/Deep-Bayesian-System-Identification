...
TS = 0.0017 # sampling time

# Universal parameters
lag = 150                           # Chosen lags for regressors
...
loadData = scio.loadmat('HEX_Data/hex.mat')
u = torch.tensor(loadData['ct'], device=TORCH_MACHINE).float()
y = torch.tensor(loadData['pt'], device=TORCH_MACHINE).float()

N = 3000
u_est, y_est, u_val, y_val = lib.select_train_test_data(u, y, N)
...
val_time = time[-1]+np.arange(u_val.shape[0]) * TS

############################### IMPORT MLP ###################################

index_lambda = float(3)              # Regularization parameter
rep_index = 6                        # Number of time to repeat experiment
...
FOLDER_HEAD = './MLP/'
folder = FOLDER_HEAD + 'HEX_results_layers_' + str(layers) + '_units_' + \
...
time_total = np.arange(u.shape[0]) * TS
...
ax.grid(True, 'major', 'x')
ax.grid(True, 'both', 'y')
# ax.set_title('One Step Ahead prediction results for HEX Benchmark.')
ax.set_ylabel(r'product $\delta$ temperature [$^\circ$ C]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_mlp0.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                    top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_mlp_pred_results_" + name + ".pdf")
...
fig_mlp1.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                         top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_mlp_sim_results_" + name + ".pdf")
...
chosen_model = ax.vlines(chosen_outer*epochs, 0, 3, color='tab:red',
...
fig_mlp2.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                         top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_mlp_training_progress_" + name + ".pdf")

######################### Plot Validation Progress ##########################
fig_mlp3 = plt.figure(figsize=[7, 4])
ax = fig_mlp3.add_subplot(1, 1, 1)
ax.axis([0, len(pred_vals), 0, 0.6])
...
fig_mlp3.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                         top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_mlp_validation_progress_" + name + ".pdf")

######################### Plot Sparsity MLP #################################
fig_mlp4 = plt.figure(figsize=[7, 4])
gs = fig_mlp4.add_gridspec(4, 4)
ax = fig_mlp4.add_subplot(gs[0:2, :])
ax.spy(np.abs(mlp_model.weights['linear_0'].detach().cpu().numpy()),
       markersize=4)
...
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_mlp_sparse_model_" + name + ".pdf")
...
ax.grid(True, 'major', 'x')
ax.grid(True, 'both', 'y')
# ax.set_title('One Step Ahead prediction results of HEX dataset.')
ax.set_ylabel(r'product $\delta$ temperature [$^\circ$ C]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_lstm0.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                    top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_lstm_pred_results_" + name + ".pdf")
...
fig_lstm1.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                          top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_lstm_sim_results_" + name + ".pdf")
...
fig_lstm2.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                          top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_lstm_training_progress_" + name + ".pdf")

######################### Plot Validation Progress ##########################
fig_lstm3 = plt.figure(figsize=[7, 4])
ax = fig_lstm3.add_subplot(1, 1, 1)
ax.axis([0, len(pred_vals), 0, 0.4])
ax.set_xticks(np.arange(0, outer_times+1, 1))
pred_prog = ax.plot(np.arange(outer_times) + 1, pred_vals, 'x',
                    color = 'tab:blue', linewidth=2,
                    label=r'One Step Ahead Prediction RMSE')
...
fig_lstm3.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                          top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_lstm_validation_progress_" + name + ".pdf")
...
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_lstm_sparse_model_" + name + ".pdf")
...
ax.grid(True, 'major', 'x')
ax.grid(True, 'both', 'y')
# ax.set_title('Simulation Comparison of HEX validation dataset.')
ax.set_ylabel(r'product $\delta$ temperature [$^\circ$ C]')
ax.set_xlabel(r'time [s]')
ax.legend(loc='upper right')
fig_sim.subplots_adjust(left=0.15, bottom=0.12, right=0.9,
                        top=0.9, wspace=0.3, hspace=0.2)
name = 'lag_' + str(lag) + '_lambda_' + str(index_lambda)
plt.savefig("./figures/hex_sim_comparison_" + name + ".pdf")