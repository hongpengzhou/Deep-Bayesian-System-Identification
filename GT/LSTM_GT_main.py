...
# Training settings
lstmLayer = 10                      # Number of hidden units per layer
...
lambdaDecayEpochs = 600             # Lambda decay each'lambdaDecayEpochs'
gammaThreshold = 1e-3               # Threshold for gamma pruning
weightThreshold = 1e-3              # Threshold for weight pruning
trainBatchSize = 1024               # Choose batch size for each train

# Other relevant data parameter Setting
onoffNormalize = False              # Whether to normalize data
...
uEst = torch.tensor(loadData['udse'], device=TORCH_MACHINE).float()
yEst = torch.tensor(loadData['ydse'], device=TORCH_MACHINE).float()
uVal = torch.tensor(loadData['udsv'], device=TORCH_MACHINE).float()
yVal = torch.tensor(loadData['ydsv'], device=TORCH_MACHINE).float()
...
# NN Structure
layers = [uEstDataPro.shape[1], lstmLayer, yEstDataPro.shape[1]]
...
for repIndex in range(repeatExperiment):
    ...
    model = RNNNetwork(layers, save_device=TORCH_MACHINE,
                        bias=bias).to(TORCH_MACHINE)
    ...
    # Folder and filename
    folder = FOLDER_HEAD + 'GT_results_layers_' + str(layersN) + '_units_' + \
             str(lstmLayer) + '/' + 'percentage_' + str(percentage) + '/'
    ...
    for outer in range(outerTimes):
        ...
        if lossTotal < minimalLoss:
            ...
            # Set convregu/noregu/bestregu sign and save best models
            if not onoffRegu:
                sign = '_noregu'
                # Save found model
                torch.save(modelDict, filename + sign + '_model.tar')
            elif outer == 0:
                sign = '_convregu' # Model already saved in first outer above
            else:
                sign = '_bestregu'
                # Override _bestregu with the new best for the experiment
